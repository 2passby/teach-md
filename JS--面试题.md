...
## async 函数的作用
在函数前加上 async 关键字，表示这是一个异步函数。它的作用是：
自动将返回值包装成 Promise；
允许在函数内部使用 await 来等待异步操作；
函数内部抛出的错误会自动变成 rejected 的 Promise。
...
## jsonp发送的请求，怎么找到对应的回调函数执行的？
  1.创建一个script标签
  2.配置script的src为发送请求的url
  3.src中配置参数callback，为请求回后执行的回调函数
  4.将script标签插到文档中，实现script的加载与执行
