...
首先优先级较高的规则会覆盖优先级较低的规则，而如果如果两个规则的优先级相同，那么后定义的规则会覆盖先定义的规则，选择器的优先级是!important > id选择器 > 类选择器 /伪类选择器/属性选择器 > 元素选择器/伪元素选择器，并且根据该规则对应选择器的个数填入（a,b,c,d），比较优先级，从高位到低位
...
1.利用定位 子绝父相，然后让子元素通过translate平移x和y轴自身一半的距离，实现居中
